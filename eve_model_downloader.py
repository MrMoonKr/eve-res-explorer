'''
    https://github.com/puffingprie/GetEveModels
    '''
...
resfileindex_file_path  = os.path.join(file_directory_path, "cfg", "resfileindex.txt")
res_models_file_path    = os.path.join(file_directory_path, "cfg", "res_models.txt")
file_types_file_path    = os.path.join(file_directory_path, "cfg", "file_types.txt")
res_file_path           = os.path.join(file_directory_path, "res/")
obj_models_file_path    = os.path.join(file_directory_path, "obj_models/")

URL_CDN                 = "https://resources.eveonline.com/"
...
def _wanted_file(res_file: str):
    if ("model/ship" in res_file and 
        ("gr2" in res_file or "ar.dds" in res_file or "no.dds" in res_file or "pmdg.dds" in res_file) 
    ):
        if ("lowdetail" not in res_file and 
            "mediumdetail" not in res_file and 
            "effect" not in res_file and 
            "strategiccruiser" not in res_file and
            "wreck" not in res_file
        ):
            return True
    else:
        return False
...
def _load_res_list():
    ...
    with open(resfileindex_file_path, "r") as file:
        ...
        for i in range(len(resFile)):
            # if ("model/ship" in resFile[i] and 
            #     ("gr2" in resFile[i] or "ar.dds" in resFile[i] or "no.dds" in resFile[i] or "pmdg.dds" in resFile[i]) 
            # ):
            #     if ("lowdetail" not in resFile[i] and 
            #         "mediumdetail" not in resFile[i] and 
            #         "effect" not in resFile[i] and 
            #         "strategiccruiser" not in resFile[i] and
            #         "wreck" not in resFile[i]
            #     ):
            #         appendedResFile.append(resFile[i])
            if ( _wanted_file( resFile[i] ) ):
                appendedResFile.append( resFile[i] )

    with open(res_models_file_path, "w") as file:
...
def make_associated_res_file():
    with open(os.path.join(file_directory_path, "associated_file_names.txt"), "r") as associated_file_names_file:
        ...
        # associated_file_names_dict = dict((identifier, ship_name) for identifier, ship_name in (i.rsplit(": ") for i in associated_file_names_list))
        associated_file_names_dict = {}
        for line in associated_file_names_list:
            parts = line.rsplit(": ", 1)
            if len(parts) != 2:
                continue
            identifier = parts[0]
            ship_name = parts[1]
            associated_file_names_dict[identifier] = ship_name

    with open(res_models_file_path, "r") as res_file:
...
def download_resource(file_line):
    try:
        ...
        relative = file_line_split[2].replace(URL_CDN, "").replace("\\", "/").strip("/")
        encoded_relative = quote(relative, safe="/-._~")
        url = f"{URL_CDN}{encoded_relative}"
        file_name = file_line_split[0]
        destination = os.path.join(res_file_path, file_name)
                ...
                with urlopen(request, timeout=30) as response, open(destination, "wb") as output:
                    ...
                    while True:
                        ...
                        if total_size > 0:
                            ...
                            print(
                                f"\rDownloading {file_name}: {percent:6.2f}% ({downloaded}/{total_size} bytes)",
                                end="",
                                flush=True
                            )
                        else:
        ...
        print("[eve][error] An error occured: ", e)
        return
...
def download_all_threads(download_type="all"):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {executor.submit(download_resource, line): line for line in file_lines}
        results = {}
        for future in concurrent.futures.as_completed(futures):
            arg = futures[future]
            results[arg] = future.result()
            completed_count += 1
            print(f"[{completed_count}/{total_count}] {results[arg]}")
    t2 = time.perf_counter()
    print(f"Done in {t2-t1} second(s)")
